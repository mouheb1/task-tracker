// -- ./src/template/ui/watermark/watermark.tsx
...
function generateSvg(options: SvgOptions): string {
  const {
    text,
    textColor,
    textSize,
  ...
  if (multiline) {
    const texts = text.split("\n").map((textByLine, index) => {
      return `<tspan x='50%' dy='${index === 0 ? "0" : lineHeight
        }'>${textByLine}</tspan>`;
    });
  ...
  return `<svg width='${size}' height='${size / 1.5
    }' xmlns='http://www.w3.org/2000/svg'>${textEl}</svg>`;
}

function calcTextRenderedRect(text: string,
  fontSize: number,
  lineHeight: string,
  fontFamily: string
): DOMRect {
...
const watermarkWrapperStyle: React.CSSProperties = {
  position: "relative",
};
...
const Watermark = ({
  ...
  useEffect(() => {
    if (image) {
      const gutteredSvg = `
        <svg xmlns='http://www.w3.org/2000/svg' width='${gutter + 100
        }' height='${gutter + 100}' viewBox='0 0 ${gutter + 100} ${gutter + 100
        }'>
          <rect x='0' y='0' width='${gutter + 100}' height='${gutter + 100
        }' fill='transparent' />
          <image href='${image}' x='${gutter / 2}' y='${gutter / 2
