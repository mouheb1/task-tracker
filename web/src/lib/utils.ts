...
export const isMenuItemActive = (menuItem: any, locationName: string): boolean => {
  if (menuItem.href && isLocationMatch(menuItem.href, locationName)) {
    return true;
  }
  if (menuItem.child) {
    return menuItem.child.some((childItem: any) => isMenuItemActive(childItem, locationName));
  }
  if (menuItem.nested) {
    return menuItem.nested.some((nestedItem: any) => isMenuItemActive(nestedItem, locationName));
  }
  return false;
};
...
export const handleError = (error: any): string => {
  ...
  // Check if there are GraphQL errors with extensions
  const graphQLError = error?.graphQLErrors?.[0]?.extensions;
  if (graphQLError) {
    const { message: extensionsErrorMessage, code } = graphQLError;

    // Use the extensions message if available
    if (extensionsErrorMessage) {
...
export const getDaysOfWeekPositions = (day: string): number[] => {
  const DAYS_CONVERTOR = {
    MONDAY: 1,
    TUESDAY: 2,
    WEDNESDAY: 3,
    THURSDAY: 4,
